...
boot_alloc(uint32_t n)
{
    ...
    if (n > 0) {
        ...
        // If we exceed available memory, panic
        if ((size_t)(PADDR(nextfree)) >= npages * PGSIZE)
            panic("boot_alloc: Out of memory");
    } else {
        // n == 0, just return the current free position
        result = nextfree;
    }
...
page_init(void)
{
	...
	// +++ Activity 1: YOUR CODE HERE
	size_t i;
	size_t first_free_page = PGNUM(PADDR(boot_alloc(0)));
    ...
    for (i = 0; i < npages; i++) {
        ...
        if (i >= PGNUM(EXTPHYSMEM) && i < first_free_page) {
            pages[i].pp_ref = 1;
            pages[i].pp_link = NULL;
            continue;
        }
...
page_free(struct PageInfo *pp)
{
    // +++ Activity 1: YOUR CODE HERE
    if (pp->pp_ref != 0)
        panic("page_free: freeing page with nonzero pp_ref");
    if (pp->pp_link != NULL)
        panic("page_free: page is already in free list");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    if (*pde & PTE_P) {
        ...
        // Need to create a new page table if create is set
        if (!create)
            return NULL;
        ...
        *pde = page2pa(new_pt) | PTE_W | PTE_U | PTE_P;
        pte_t *pt = (pte_t *)KADDR(PTE_ADDR(*pde));
        return &pt[PTX(va)];
    }
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
    ...
    if (*pte & PTE_P) {
        // There was already a page mapped here
        struct PageInfo *old_pp = pa2page(PTE_ADDR(*pte));
        if (old_pp != pp) {
            page_remove(pgdir, va);
        } else {
            // Same page re-inserted, decrement the increment we just did
            // because page_remove won't happen
            pp->pp_ref--;
        }
    }
...
page_remove(pde_t *pgdir, void *va)
{
    ...
    // Decrement refcount and free if necessary
    page_decref(pp);
    // Clear the PTE
    *pte = 0;
    // Invalidate TLB
